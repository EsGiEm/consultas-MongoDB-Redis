...
app.post('/buscar', async (req, res) => {
  ...
  try {
    ...
    const startTimeRedis = performance.now();  // Tiempo de inicio Redis
    const cacheResults = await redisClient.get(cacheKey);
    const endTimeRedis = performance.now();  // Tiempo de inicio Redis


    if (cacheResults) {
      console.log('📦 Respuesta obtenida desde Redis');
      console.log(`Consulta a Redis ejecutada en ${(endTimeRedis - startTimeRedis).toFixed(2)}ms`);
    ...
    const startTimeMongo = performance.now();  // Tiempo de inicio MongoDB
    const resultados = await contactosCollection.find(query).toArray();
    const endTimeMongo = performance.now();  // Tiempo de fin MongoDB
    console.log(`Consulta a MongoDB ejecutada en ${(endTimeMongo - startTimeMongo).toFixed(2)}ms`);
